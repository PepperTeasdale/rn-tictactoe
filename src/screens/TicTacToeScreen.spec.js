...
describe('<TicTacToeScreen/>', () => {
  ...
  it('displays the winner when a win condition is met', () => {
    ...
    expect(instance.findAll(child => child.props.children === 'Player 1 Wins!').length).toBe(0)
...
describe(gameReducer, () => {
  describe('MOVE_OCCURRED', () => {
    ...
    it("detects when a player has 3 in second row diagonally from bottom left to top right ", () => {
      const initialState = {
        currentTurn: 0,
        board: [
          [EMPTY_SQUARE,O_SQUARE,EMPTY_SQUARE],
